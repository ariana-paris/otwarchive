  ...
  def import_work(archivist, external_work)
    ...
    if work_status == :ok
      ...
      if errors.empty?
        @some_success = true
        work_status = :created
        work_url = work_url(@works.first)
        work_messages << "Successfully created work \"" + @works.first.title + "\"."
      else
        @some_errors = true
        work_status = :unprocessable_entity
        work_messages << errors
      end
    end
  ...
  def options(archivist, params)
    {
      ...
      work: {
        fandom_string: params[:fandoms],
        warning_strings: params[:warnings],
        character_string: params[:characters],
        rating_string: params[:rating],
        relationship_string: params[:relationships],
        category_string: params[:categories],
        freeform_string: params[:additional_tags],
      },
